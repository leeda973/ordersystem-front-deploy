...
import axios from 'axios';
    ...
    export default{
        ...
        methods:{
            ...
            fileUpload(event){
                // 여러개의 이미지일 경우 for문을 실행하여 productImage에 리스트형태로 push
            ...
            async productCreate(){
                try{
                    ...
                    // image를 아예 append 시키지 않으면, 백엔드 dto에 null로 매핑되나, null을 append 시키면 백엔드에서 타입에러발생
                    if(this.productImage){
                        data.append("productImage", this.productImage);
                    }
                    const result = await axios.post(`${process.env.VUE_APP_API_BASE_URL}/product/create`, data);
